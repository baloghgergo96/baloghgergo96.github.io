...
const log = (locale = 'hu-HU', currencyLocale = 'HUF') => {
    ...
    const amount = 4533;
    const localizedAmount = new Intl.NumberFormat(locale, {style: 'currency', currency: currencyLocale}).format(amount);
...
console.log(log('en-GB', 'GBP'));
console.log(traditionalLog());
...
//A collectUsers függvény objektumok tömbjét kapja, minden objektumban van egy users tulajdonásg, ami number típusú.
//Adja vissza a függvény a users objektumokban tárolt users-számok összegét.

const collectUsers = (list = []) => {
    //accumulator
    let sumOfUsers = 0;
